...
class MemoryEfficientSoftDiceLoss(nn.Module):
    def __init__(self, apply_nonlin: Callable = None, batch_dice: bool = False, do_bg: bool = True, smooth: float = 1.,
        ...
        这段代码定义了一个名为MemoryEfficientSoftDiceLoss的Dice Coefficient损失函数类。
        该类继承自nn.Module类，用于计算网络输出和目标标签之间的Dice Coefficient损失。
        构造函数中的参数包括apply_nonlin、batch_dice、do_bg、smooth和ddp。
        其中apply_nonlin表示应用于网络输出的非线性函数。
        batch_dice表示是否对每个样本计算Dice损失。do_bg表示是否包括背景类别。
        smooth表示平滑因子。ddp表示是否使用分布式数据并行（Distributed Data Parallel）。
        forward方法接受网络输出x和目标标签y作为输入，并根据apply_nonlin对网络输出进行非线性变换。
        然后根据do_bg和loss_mask参数选择是否包括背景类别和损失掩码。
        接着将目标标签转换为one-hot编码，并根据loss_mask对其进行加权。
        然后计算交集、预测值总和和目标值总和，并根据ddp和batch_dice参数对其进行处理。
        最后计算Dice Coefficient并返回其负值作为损失。
        这段代码的作用是定义了一个内存效率较高的Dice Coefficient损失函数，
        用于训练模型时计算损失并更新网络参数。
        """
        #损失掩码指的是什么？ 有什么作用？--只是bool标签--确定是否只对部分类别进行损失值计算

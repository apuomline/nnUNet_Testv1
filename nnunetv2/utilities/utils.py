...
def get_filenames_of_train_images_and_targets(raw_dataset_folder: str, dataset_json: dict = None):
    ...
    if 'dataset' in dataset_json.keys():
        ...
        """
        否则，说明数据集是分割的形式。
        使用get_identifiers_from_splitted_dataset_folder函数
        从raw_dataset_folder/imagesTr目录中获取标识符列表，
        该函数会根据文件扩展名dataset_json['file_ending']来获取标识符。
        使用create_lists_from_splitted_dataset_folder函数
        从raw_dataset_folder/imagesTr目录中获取图像列表，
        该函数会根据文件扩展名dataset_json['file_ending']和标识符列表来获取图像列表。
        使用列表推导式生成标签路径列表segs，
        将其与标识符列表和图像列表一一对应，并将它们作为键值对存储在dataset字典中。
        返回最终的dataset字典，其中包含了训练图像和目标文件的文件名
        """

...
def filter_available_models(model_dict: Union[List[dict], Tuple[dict, ...]], dataset_name_or_id: Union[str, int]):
    """
    这段代码定义了一个名为`filter_available_models`的函数，用于过滤可用的模型。

    函数接受两个参数：
    - `model_dict`：包含模型信息的字典列表或元组。
    - `dataset_name_or_id`：数据集的名称或ID。

    函数首先创建一个空列表`valid`，用于存储有效的模型。

    然后，对于每个模型，它首先根据给定的`dataset_name_or_id`和模型的`plans`属性构建一个`PlansManager`对象。然后，它检查模型的`configuration`属性是否在`plans_manager`的可用配置列表中。如果不在，则打印相应的错误信息，并继续下一个模型。

    接下来，它构建了预期的模型输出文件夹路径，并检查该文件夹是否存在。如果不存在，则引发`RuntimeError`异常，指示该模型没有输出文件夹。

    最后，如果模型通过了所有检查，它将该模型添加到`valid`列表中。

    最终，函数返回`valid`列表，其中包含所有通过检查的模型。
    """
    valid = []
...
def generate_inference_command(dataset_name_or_id: Union[int, str], configuration_name: str,
                               ...
                               save_npz: bool = False):
    """
    这段代码定义了一个名为`generate_inference_command`的函数，用于生成执行推断（inference）的命令。

    函数接受以下参数：
    - `dataset_name_or_id`：数据集的名称或ID。
    - `configuration_name`：配置的名称。
    - `plans_identifier`：计划的标识符，默认为'nnUNetPlans'。
    - `trainer_name`：训练器的名称，默认为'nnUNetTrainer'。
    - `folds`：折叠的列表或元组，默认为(0, 1, 2, 3, 4)。
    - `folder_with_segs_from_prev_stage`：前一阶段分割结果的文件夹，默认为None。
    - `input_folder`：输入文件夹的名称，默认为'INPUT_FOLDER'。
    - `output_folder`：输出文件夹的名称，默认为'OUTPUT_FOLDER'。
    - `save_npz`：一个布尔值，指示是否保存概率结果，默认为False。

    函数首先根据给定的参数生成fold的字符串表示。然后，它检查计划文件夹中是否存在'previous_stage'配置。如果存在，它将生成执行前一阶段推断的命令，并将其添加到`predict_command`中。

    接下来，它使用给定的参数生成执行当前阶段推断的命令，并将其添加到`predict_command`中。最后，它根据需要添加`folder_with_segs_from_prev_stage`和`save_npz`参数。
...
def find_best_configuration(dataset_name_or_id,
                            ...
                            strict: bool = False):
    """
    这段代码定义了一个名为`find_best_configuration`的函数，用于找到最佳的配置。

函数接受以下参数：
- `dataset_name_or_id`：数据集的名称或ID。
- `allowed_trained_models`：允许的训练模型列表或元组，默认为`default_trained_models`。
- `allow_ensembling`：一个布尔值，指示是否允许集成，默认为True。
- `num_processes`：并行处理的进程数，默认为`default_num_processes`。
- `overwrite`：一个布尔值，指示是否覆盖现有结果，默认为True。
- `folds`：折叠的列表或元组，默认为(0, 1, 2, 3, 4)。
- `strict`：一个布尔值，指示是否严格检查模型的输出文件夹是否存在，默认为False。

函数首先将`dataset_name_or_id`转换为数据集名称，并创建一个空字典`all_results`。

然后，它使用`filter_available_models`函数过滤出可用的训练模型，并将结果赋值给`allowed_trained_models`。

接下来，对于每个允许的训练模型，它获取输出文件夹路径，并检查该文件夹是否存在。如果不存在且`strict`为True，则引发`RuntimeError`异常。

然后，它获取模型的标识符，并创建合并的输出文件夹路径。接着，它调用`accumulate_cv_results`函数来累积交叉验证结果，并将结果添加到`all_results`字典中。

如果允许集成，则对于每对模型，它获取输出文件夹路径，并创建集成的输出文件夹路径。然后，它调用`ensemble_crossvalidations`函数来生成集成预测结果，并计算集成预测结果的评估指标。最后，它
    """
